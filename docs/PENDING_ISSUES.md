...
- **决定性线索**：设备上手动 `launchctl bootstrap system .../com.openssh.sshd.plist` 报 **`failed to patch plist`**。这是 roothide 的 `launchctl` 包装器在加载第三方 daemon 时"打补丁"（解析 jbroot 路径 / 把可执行文件塞进动态 trustcache）那步失败。
- **更正（重要）**：进一步发现，此时设备上的 plist 已被（失败的 Sileo openssh 重装的解包阶段）**还原成原版（`ssh`+`2222`）**，但 `launchctl bootstrap` **对原版 plist 仍报 `failed to patch`**。⇒ **patch 失败不是 plist 内容的问题**（我的改写虽是导火索，但还原后依旧失败），而是**设备上 openssh 的安装/trustcache 状态被反复失败的安装弄坏了**（half-configured：Sileo 重装因 extrainst_ 老式 launchctl + set -e 失败而中断在 unpacked-not-configured）。
- **持久化陷阱**：openssh 的预装（`dpkg -i` 覆盖回原版 plist）只在**首次激活**跑（`if fileExists /prep_bootstrap.sh`，仅全新 bootstrap 存在）；已 bootstrap 的设备**重越狱走 else 分支不重装 openssh**，所以坏状态跨 rebuild/重越狱持久存在。
- **恢复**：**Dopamine → Restore Rootfs**（清空 bootstrap → 下次越狱变全新首次激活 → 重新 `dpkg -i openssh` + 重建 trustcache + 原版 plist → patch 成功 → sshd 起在 22）。配新版（preload-16 已停用）即彻底修好。手动 NewTerm 难以可靠修复 trustcache 半坏状态。
